...
try {
  ...
  if (!global.config.EMAIL) {
    logger.err(`please enter your email address in ${chalk.blueBright('Config.json')} file`)
    process.exit(0);
  }
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            const configures = require(`../../Config.json`);
            ...
            global.loading.err(`${chalk.hex('#ff7100')(``)}failed to deploy ${chalk.hex("#FF00FF")(command)} ` + error + '\n', "command");
          }
        }
      })(),
        ...
        for (const ev of events) {
          try {
            ...
            if (!config || !config.name || !run) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#00FFFF")(ev)} module is not in the correct format. `, "event");
              continue;
            }
